...
const handleDelete = async (commentId) => {
  ...
  return (
    <div className='max-w-3xl mx-auto w-full p-3' >{currentUser ? (
<div className='flex items-center gap-1 my-5 text-gray-500 text-sm' >
  <p>Signed in as:</p>
  <img className='h-5 w-5 object-cover rounded-full' src={currentUser.profilePicture} alt='DP' />
  <Link className='text-xs text-cyan-600 hover:underline'  to={'/dashboard?tag=profile'} >@{currentUser.username}</Link>
</div>
        ...
        >
          ...
          <div className='flex justify-between items-center mt-5'>
            <p className='text-gray-500 text-xs'>
              {comment &&   200 - comment.length} characters remaining
            </p>
